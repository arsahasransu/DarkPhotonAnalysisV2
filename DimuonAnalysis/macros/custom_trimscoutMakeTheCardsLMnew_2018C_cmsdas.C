...
void custom_trimscoutMakeTheCardsLMnew_2018C_cmsdas(string treepath = "scout_2.root", const char* outfilename = "./custom_scout_cmsdas.root", bool isMC=false) {
    ...
    TH2F* vertexPositions = new TH2F("vertexPositions","vertexPositions",400,-20,20,400,-20,20);
    TH2F* vertexPositions_dimuons = new TH2F("vertexPositions_dimuons","vertexPositions of muon pairs",400,-20,20,400,-20,20);
    ...
    for(unsigned int f=0; f<filesToRun.size(); f++){
        ...
        while(reader.Next()) {
            ...
            // VERTEX CONSTRAINT
            if( (  sqrt( ((*vtxX)[0] - BSx)*((*vtxX)[0] - BSx) + ((*vtxY)[0] - BSy)*((*vtxY)[0] - BSy) )/sqrt( ((*vtxYError)[0]*(*vtxYError)[0]) + ((*vtxXError)[0]*(*vtxXError)[0]))  ) < 1.2 ){
                passPVconstraintSig = true;
            }
            passPVconstraintSig = true;

            vertexPositions->Fill((*vtxX)[0],(*vtxY)[0]);

            // DIMUON MASS DISTRIBUTION
            if(m1ch*m2ch<0. && passPVconstraintSig && m1pt>slidePt1 && m2pt>slidePt2 && maxEta<1.9){
                dimuonMass->Fill(mass);
                vertexPositions_dimuons->Fill((*vtxX)[0],(*vtxY)[0]);
            }
            if(m1ch*m2ch<0. && passPVconstraintSig && m1pt>slidePt1 && m2pt>slidePt2 && maxEta<1.9 && mass>2.7 && mass<3.4){
                dimuonMassJPsi->Fill(mass);
            }

            // SLIDING MASS WINDOW - FILLING HISTS/DATACARDS
            float ma=0.2;
            for(int j=0; j<400.; j++){
                ma = ma+(ma*0.01); 
    ...
    vertexPositions->Write();
    vertexPositions_dimuons->Write();
    for(int j=0; j<400.;j++){
